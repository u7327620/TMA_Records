...
class Record:
    ...
    def to_txt(self):
        return f"{self.player}: {self.wins}-{self.draws}-{self.losses}"
...
if __name__ == "__main__":
    ...
    for record in sorted(records.values(), key=lambda x: len(x.matches)):
        print(record)
    ...
    path = os.path.join(os.getcwd(), "records", "records.json")
